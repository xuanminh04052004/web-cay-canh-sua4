...
const ProductDetail = () => {
  ...
  let plant: (Plant & { sellerId?: string }) | undefined = products.find((p) => p.id === Number(id));
  ...
  const relatedPlants = products.filter((p) => p.category === plant.category && p.id !== plant.id).slice(0, 4);
