...
import { recordOrderSale } from '@/lib/order-service';
  ...
  const handleSubmit = async () => {
    ...
    try {
      await recordOrderSale(
        sharedOrderId.toString(),
        total,
        items.map(item => ({
          plantId: typeof item.plant.id === 'string' && (item.plant.id as string).startsWith('sp_') 
                   ? parseInt((item.plant.id as string).replace('sp_', '')) + 10000 
                   : Number(item.plant.id),
          quantity: item.quantity,
          price: item.plant.price
        }))
      );
    } catch (error) {
      console.warn("Failed to record global sale logic for persistent data", error);
    }
