...
import { 
  fetchUserOrders as fetchUserOrdersFromApi,
  createUserOrder,
  cancelUserOrder,
...
/**
 * Map Vietnamese order status to English status
 */
function mapUserOrderStatus(vietnameseStatus: string): 'pending' | 'confirmed' | 'shipping' | 'delivered' | 'cancelled' {
...
export const UserProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    if (authUser) {
      ...
      // Fetch user orders from MockAPI
      fetchUserOrdersFromApi(authUser.id)
        .then(apiOrders => {
          if (apiOrders && apiOrders.length > 0) {
            // Convert API orders to display format
            const convertedOrders = apiOrders.map(apiOrder => ({
              id: apiOrder.id,
              date: apiOrder.date,
              status: mapUserOrderStatus(apiOrder.status) as any,
              ...
              timeline: [{
                ...
                date: apiOrder.date,
                description: `Trạng thái: ${apiOrder.status}`,
              }],
            })) as Order[];
            ...
            setOrders(prev => {
              ...
              const newOrders = convertedOrders.filter(o => !existing.has(o.id));
              return [...prev, ...newOrders];
            });
          }
        })
        .catch(error => {
          console.warn('Failed to fetch user orders from API:', error);
        });
    }
  }, [authUser]);
  ...
  const addOrder = async (order: Omit<Order, 'id' | 'timeline'> & { id?: string }) => {
    let finalId = order.id || Date.now().toString();
    ...
    // Push order to User MockAPI using the synchronized ID
    if (authUser) {
      const apiOrder = {
        id: finalId, // Try forcing MockAPI to use the same ID
        userId: authUser.id,
      ...
      createUserOrder(apiOrder)
        .catch(error => {
          console.warn('Failed to create user order in API:', error);
        });
    }
  };
  ...
  const cancelOrder = (orderId: string, reason: string): boolean => {
    ...
    if (order && (order.status === 'pending' || order.status === 'confirmed')) {
      ...
      // Cancel order in MockAPI
      cancelUserOrder(orderId, reason)
        .catch(error => {
          console.warn('Failed to cancel user order in API:', error);
        });
